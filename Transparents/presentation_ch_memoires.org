...
- Il ne peut pas y avoir de récriture.
...
- Les mémoires disponibles sur le marché optent souvent pour une combinaison des signaux de contrôle, avec un seul signal qui détermine le sens de l'action, comme on peut le voir dans le tableau [[tab-controle_memoire]].
...
- Leurs usages typiques sont surtout déterminés par la façon de les configurer (on dit couramment /programmer/, même s'il s'agit d'un intervention au niveau du matériel).
